...
function load_data(data_path, data_filename)
    # function to load and preprocess Swiss Groundwater data of a specific well

    # ==========================================================================
    # Load data from file
    path_alldata = joinpath(data_path, data_filename)
    df = CSV.File(path_alldata, delim=',', header = true)|> DataFrame

    # =================================================================
    # adapt dates

    df_dates = df[!,"Date"]
    # df_date_length = length(df_dates)
    df_dates = Date.(df_dates)#Date.(SubString.(df_dates,1,df_date_length), "yyyy mm dd")
    df[!,"Date"] = df_dates
